 ...
 */
@Slf4j
@Api(tags = {"aftersale::订单售后下载接口"})
@Controller
@RequestMapping("/aftersale/order-after-sale-download")
public class OrderAfterSaleDownloadController {
    ...
    public void downloadDetail(
            ...
            @ApiParam(name = "orderAfterSaleQO", value = "查询条件")
                    OrderAfterSaleQO orderAfterSaleQO) throws IOException {

        if(ObjectUtil.isNull(orderAfterSaleQO.getBusinessId())) {
            throw new WeikbestException(ResultConstant.VALID_FAIL.getCode(), "关联商户ID不为空！");
        }

        // 获取excel下载数据
        List<OrderAfterSaleDetailExcel> orderAfterSaleDetailExcelList = orderAfterSaleService.downloadDetail(orderAfterSaleQO);

        response.setContentType("application/vnd.openxmlformats-officedocument.spreadsheetml.sheet");
        response.setCharacterEncoding("utf-8");
        // 这里URLEncoder.encode可以防止中文乱码 当然和easyexcel没有关系
        String fileName = URLEncoder.encode(String.format("售后列表_%s.xlsx", DateUtil.now()), "UTF-8");
        response.setHeader("Content-disposition", "attachment; filename=" + fileName);
        response.setHeader("filename", fileName);
        EasyExcel.write(response.getOutputStream(), OrderAfterSaleDetailExcel.class).sheet("售后明细").doWrite(orderAfterSaleDetailExcelList);
    }
}
