 ...
 * @since :2022/10/15
 * <p>
 * 售后单客户寄回商品后商家确认收货处理过期延时任务
 */
@Slf4j
@Component
public class OrderAfterSaleBusinessConfirmCustomerDeliveryTimeoutDelayTaskProcess extends DelayTaskProcess {

    /**
     * 售后单延时队列前缀
     */
    public static final String TASK_PREFIX = RedisKey.DelayTask.DELAY_ORDER_AFTER_SALE_BUSINESS_CONFIRM_CUSTOMER_DELIVERY_TIMEOUT_KEY;
    ...
    public OrderAfterSaleBusinessConfirmCustomerDeliveryTimeoutDelayTaskProcess() {
...
//        long timeout = dealService.findDeal().getRefundBusinessTimeout() * 24 * 60 * 60 * 1000L;
        delayQueueManager.putTask(taskId, TIME_OUT_7_DAY);
    }
    ...
    /**
     * @param taskId 任务ID = DELAY_ORDER_AFTER_SALE_BUSINESS_CONFIRM_CUSTOMER_DELIVERY_TIMEOUT:售后单ID:售后单状态
     */
    @Override
    public void doProcess(String taskId) {
        ...
        // 执行客户寄回商品后商家确认收货处理过期延时任务
        orderAfterSaleService.businessConfirmCustomerDeliveryTimeout(id, afterSaleStatus);
    }

    @Override
    public String getName() {
        return "售后单客户寄回商品后商家确认收货处理过期延时队列";
    }
