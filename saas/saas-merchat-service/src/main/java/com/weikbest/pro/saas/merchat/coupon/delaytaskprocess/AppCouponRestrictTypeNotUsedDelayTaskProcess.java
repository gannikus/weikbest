...
@Slf4j
@Component
public class AppCouponRestrictTypeNotUsedDelayTaskProcess extends DelayTaskProcess {

    /**
     * 优惠券领用状态未使用队列前缀
     */
    public static final String TASK_PREFIX = RedisKey.DelayTask.DELAY_COUPON_RESTRICT_TYPE_NOTUSED_KEY;

    @Resource
    private CustCouponRestrictService custCouponRestrictService;


    public AppCouponRestrictTypeNotUsedDelayTaskProcess() {
        super.setTaskPrefix(TASK_PREFIX);
        super.setTaskLockPrefix(RedisKey.Lock.LOCK_COUPON_RESTRICT_KEY);
    }

    /**
     * 将优惠券领券加入延时队列，超时时间：优惠券用券开始时间
     *
     * @param id      优惠券领券ID
     * @param timeout 优惠券用券开始时间
     */
    ...
    /**
     * @param taskId 任务ID = RedisKey.DELAY_COUPON_RESTRICT_TYPE_NOTUSED_KEY:优惠券领券ID
     */
    @Override
    public void doProcess(String taskId) {
        ...
        // 执行优惠券领券状态为未使用
        custCouponRestrictService.couponRestrictTypeNotUsedExecute(id);
    }

    @Override
    public String getName() {
        return "优惠券领券状态未使用延时队列";
    }
