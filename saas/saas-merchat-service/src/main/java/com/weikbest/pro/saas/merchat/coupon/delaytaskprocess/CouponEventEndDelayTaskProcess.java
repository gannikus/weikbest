...
@Slf4j
@Component
public class CouponEventEndDelayTaskProcess extends DelayTaskProcess {

    /**
     * 优惠券活动生效队列前缀
     */
    public static final String TASK_PREFIX = RedisKey.DelayTask.DELAY_COUPON_EVENT_END_KEY;

    @Resource
    private CouponService couponService;

    public CouponEventEndDelayTaskProcess() {
        super.setTaskPrefix(TASK_PREFIX);
        super.setTaskLockPrefix(RedisKey.Lock.LOCK_COUPON_KEY);
    }

    /**
     * 将订单加入延时队列，超时时间：优惠券活动结束时间
     *
     * @param id      优惠券ID
    ...
    /**
     * @param taskId 任务ID = RedisKey.DELAY_COUPON_EVENT_END_KEY:优惠券ID
     */
    @Override
    public void doProcess(String taskId) {
        ...
        // 执行优惠券活动结束处理
        couponService.couponEventEndExecute(id);
    }
